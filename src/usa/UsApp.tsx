...
import { getCountyFeatures } from './map';
import { dayOfYearToISO, daysInYear, formatDateReadable } from './date';
import { computeStateSunsets, type SunsetData } from './sun';
import './usa.css';
...
function App() {
  ...
  // Page title for the /USA route
  useEffect(() => {
    document.title = 'SunMap — US Sunset Time Visualization';
  }, []);
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- legacy app restored as-is; the flag must flip before the rAF compute below
    setIsComputing(true);
    ...
    <div className="app">
      ...
      <footer className="app-footer">
        ...
        <p className="footer-year">
          Year: {YEAR} · Contiguous US only · <a href="/">🌍 global view</a>
        </p>
      </footer>
    </div>
  );
